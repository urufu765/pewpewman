...
public class NewMilkSoda : Card, IRegisterable
{
    ...
    public static void Register(IPluginPackage<IModManifest> package, IModHelper helper)
    {
        ...
        helper.Content.Cards.RegisterCard(new CardConfiguration
        {
            ...
            Name = ModEntry.Instance.AnyLocalizations.Bind(["Weth", "card", "Token", "MilkSoda", "name"]).Localize,
            Art = MilkSoda_BG
    ...
    {
        return Unavailable ? [new ASodaDescription{description = ModEntry.Instance.Localizations.Localize(["Weth", "card", "Token", "MilkSoda", "empty"])}] :
        [
            new ADummyAction(),
            new ADummyAction(),
            ModEntry.Instance.KokoroApi.V2.SpoofedActions.MakeAction(
                new APseudoPulsedriveGiver
                {
                    ...
                    descriptions = [
                        ...
                        ("milksodaWarning", ModEntry.Instance.Localizations.Localize(["Weth", "card", "Token", "MilkSoda", "flip"], new {
                            shaker = GetShakenAmount(Special)
                        })),
                        //("milksodaShakes", string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "card", "Common", "MilkSoda", "descc"]), Shakes)),
                        //("milksodaprobabilities", GetEvenMoreTooltips()),
                        //("milksodaprobabilities2", string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "card", "Common", "MilkSoda", "descp"]), GetMultiplier())),
    ...
    private string? GetDescription()
    {
        if (Used)
        {
            return ModEntry.Instance.Localizations.Localize(["Weth", "card", "Token", "MilkSoda", "spent"]);  // Used description
        }
        if (Exploded)
        {
            return ModEntry.Instance.Localizations.Localize(["Weth", "card", "Token", "MilkSoda", "exploded"], new
            {
                shaker = GetShakenAmount(Special)
        ...
        if (Demo)
        {
            return ModEntry.Instance.Localizations.Localize(["Weth", "card", "Token", "MilkSoda", ((int)upgrade == 3 ? "demoC" : "demo")], new
            {
        ...
        // if (flipped)
        // {
        //     return ModEntry.Instance.Localizations.Localize(["Weth", "card", "Token", "MilkSoda", "flip"], new
        //     {
        //         shaker = GetShakenAmount(upgrade)
        //     });
        // }
        // return ModEntry.Instance.Localizations.Localize(["Weth", "card", "Token", "MilkSoda", "desc"], new
        // {
        //     amount = Shakes,
    ...
    {
        return ModEntry.Instance.Localizations.Localize(["Weth", "card", "Token", "MilkSoda", (int)(upgrade) == 3 ? "tooltipC" : "tooltip"], new
        {
            amount = Shakes,
            rate = (int)(GetMultiplier() * 100)
        });

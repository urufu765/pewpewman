...
public class SpaceRelics2 : Artifact
{
    ...
    public override List<Tooltip>? GetExtraTooltips()
    {
        ...
        if (!DontDisplayDescription)
        {
            ...
            {
                Description = ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Unreleased", GetType().Name, "clair"])
            });
        }
        if (ObtainPulsedrive > 0)
        {
    ...
    public List<Tooltip> GetVanillaStatusIconNNamez(Dictionary<Status, int> relics, int minAmountToShow = 1, bool bracketing = false)
    {
        ...
        foreach (KeyValuePair<Status, int> relic in relics)
        {
            if (relic.Value >= minAmountToShow)
            {
                ...
                {
                    Title = string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "fake_status", $"{relic.Key}", "desc"]), bracketing? $"(<c=keyword>{relic.Value}</c>)" : $"<c=keyword>{relic.Value}</c>"),

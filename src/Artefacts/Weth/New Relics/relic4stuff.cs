...
public abstract class WethRelicFour : Artifact
{
    ...
    public override List<Tooltip>? GetExtraTooltips()
    {
        ...
        {
            Description = ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Unreleased", GetType().Name, "flavour"])
        }];
    }
...
public abstract class WethRelicFourFake : Artifact
{
    ...
    public override List<Tooltip>? GetExtraTooltips()
    {
        string extra = ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Unreleased", RealRelicType.Name, Special? "extras_special":"extras"]);
...
public static class WethRelicFourHelpers
{
    ...
    public static Tooltip RelicTooltip(Type relic, int n = 1, bool icon = true, bool micro = false, bool special = false)
    {
        if (micro) // Make it so that event relics ignore the numbering maybe
        {
            ...
            {
                Title = $"<c=keyword>{n}</c> <c=artifact>{ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Unreleased", relic.Name, "title"])}</c>",
        ...
        {
            Icon = icon ? ModEntry.Instance.NewRelicIcons[relic] : null,
            Title = ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Unreleased", relic.Name, "name"]),
            TitleColor = Colors.artifact,
            Description = ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Unreleased", relic.Name, special? "desc_special":"desc"]),
            vals = [n]
        };
    }
}
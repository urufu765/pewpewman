...
public class SpaceRelics : Artifact
{
    ...
    public override List<Tooltip>? GetExtraTooltips()
    {
        ...
        // if (ObtainPulsedrive > 0)
        // {
        //     thing += string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "fake_status", "pulsedrive", "desc"]), $"<c=keyword>{ObtainPulsedrive}</c>") + "\n";
        // }
        // foreach (KeyValuePair<Status, int> relic in Relics)
        // {
        //     if (relic.Value > 0)
        //     {
        //         thing += string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "fake_status", $"{relic.Key}", "desc"]), $"<c=keyword>{relic.Value}</c>") + "\n";
        //     }
        // }
        ...
        if (ObtainPulsedrive > 0)
        {
            ...
            {
                Title = string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "fake_status", "pulsedrive", "desc"]), $"<c=keyword>{ObtainPulsedrive}</c>"),
        ...
        foreach (KeyValuePair<Status, int> relic in Relics)
        {
            if (relic.Value > 0)
            {
                ...
                {
                    Title = string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "fake_status", $"{relic.Key}", "desc"]), $"<c=keyword>{relic.Value}</c>"),

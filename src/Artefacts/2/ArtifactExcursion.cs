...
public class TerminusMilestone : TheTerminus
{
    ...
    public override List<Tooltip>? GetExtraTooltips()
    {
        ...
        if (Stage < Goals.Count)
        {
            ...
            {
                Description = ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Tooltips", "MilestoneStart"])
            });
            for (int x = Stage; x < Goals.Count; x++)
            {
                tips.Add(new GlossaryTooltip("terminusmilestone." + x)
                {
                    Description = ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Tooltips", "Milestone", x.ToString()])
                });
        ...
        else
        {
            ...
            {
                Description = ModEntry.Instance.Localizations.Localize(["Weth", "artifact", "Tooltips", "MilestoneEnd"])
            });
        }
        return tips;
    }

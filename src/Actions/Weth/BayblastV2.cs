    ...
    public override List<Tooltip> GetTooltips(State s)
    {
        ...
        if (flared)
        {
            tooltips.Add(new GlossaryTooltip("actiontooltip.flaredbayblast")
            {
                ...
                Title = ModEntry.Instance.Localizations.Localize(["Weth", "action", "FlaredBayblast", "name"]),
                TitleColor = Colors.action,
                Description = string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "action", "FlaredBayblast", "desc"]), $"<c=boldPink>{Card.GetActualDamage(s, 1, !fromPlayer)}</c>", $"<c=boldPink>{this.range}</c>")
            });
        }
        else 
        {
            tooltips.Add(new GlossaryTooltip("actiontooltip.bayblasttwo")
            {
                ...
                Title = ModEntry.Instance.Localizations.Localize(["Weth", "action", "BayblastV2", "name"]),
                TitleColor = Colors.action,
                Description = string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "action", "BayblastV2", "desc"]), $"<c=boldPink>{Card.GetActualDamage(s, 1, !fromPlayer)}</c>", $"<c=boldPink>{this.range}</c>")
            });
        }
        if (bubbleLeech)
        {
            tooltips.Add(new GlossaryTooltip("actiontooltip.bubbleleech")
            {
                ...
                Title = ModEntry.Instance.Localizations.Localize(["Weth", "action", "BubbleLeech", "name"]),
                TitleColor = Colors.drone,
                Description = ModEntry.Instance.Localizations.Localize(["Weth", "action", "BubbleLeech", "desc"])
            });
        }
        if (harmless)
        {
            tooltips.Add(new GlossaryTooltip("actiontooltip.harmlessstuff")
            {
                ...
                Title = ModEntry.Instance.Localizations.Localize(["Weth", "action", "Harmless", "name"]),
                TitleColor = Colors.action,
                Description = ModEntry.Instance.Localizations.Localize(["Weth", "action", "Harmless", "desc"])
            });
        }
        if (s.route is Combat && !HaveWeGotAnyMissileBays(s))
        {
            ...
            {
                Description = ModEntry.Instance.Localizations.Localize(["Weth", "action", "BayBlastFail", "desc"])
            });
        }
        return tooltips;
    }

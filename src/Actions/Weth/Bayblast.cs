    ...
    public override List<Tooltip> GetTooltips(State s)
    {
        ...
        if (wide)
        {
            tooltips.Add(new GlossaryTooltip("actiontooltip.widebayblast")
            {
                ...
                Title = ModEntry.Instance.Localizations.Localize(["Weth", "action", "WideBayblast", "name"]),
                TitleColor = Colors.action,
                Description = string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "action", "WideBayblast", "desc"]), $"<c=boldPink>{Card.GetActualDamage(s, 1, targetPlayer)}</c>")
            });
        }
        else 
        {
            tooltips.Add(new GlossaryTooltip("actiontooltip.bayblast")
            {
                ...
                Title = ModEntry.Instance.Localizations.Localize(["Weth", "action", "Bayblast", "name"]),
                TitleColor = Colors.action,
                Description = string.Format(ModEntry.Instance.Localizations.Localize(["Weth", "action", "Bayblast", "desc"]), $"<c=boldPink>{Card.GetActualDamage(s, 1, targetPlayer)}</c>")
            });
        }
        if (bubbleLeech)
        {
